		...
		while(run) {
			...
			// 3번(쇼핑) 선택
				...
				if(login != null) {
					...
					while(item != 4) {
						...
						// 장바구니 배열에 id인덱스 & item인덱스 담기
						for(int i=0; i<items.length; i++) {
							jang[itemCount][0] = idIndex; // 해당 아이디 장바구니 배열[i][0]에 넣기
							cart[itemCount][0] = idIndex; // 해당 아이디 장바구니 배열[i][0]에 넣기
							if(item == i) {
								jang[itemCount][1] = i; // 선택한 상품 인덱스 장바구니 배열에 담기
								cart[itemCount][1] = i; // 선택한 상품 인덱스 수량 배열에 담기
								itemCount ++;
							}
						}
			...
			// 4번(장바구니) 선택
				...
				if(login != null) {
					...
					System.out.println("사과: "+itemCount00+"개");
					System.out.println("바나나: "+itemCount01+"개");
					System.out.println("딸기: "+itemCount02+"개");
				}
				// 로그아웃 상태로 선택(실행안됨)
				else {
					System.out.println("로그인을 먼저 해주세요.");
